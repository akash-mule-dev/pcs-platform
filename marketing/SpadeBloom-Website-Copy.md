...
**Brand colors:** navy `#0E1A2B`, blue `#2563EB`, teal `#0D9488`.
...
### Social proof strip *(placeholder until you have logos/quotes)*
"From paper travelers to a live floor in under a month." — *add first customer quote*
*(Until then, use: "Built with working metal-fab floors, not a whiteboard.")*

### Problem
**Headline:** Running a growing floor on paper? You're flying half-blind.
**Body:** Paper travelers and spreadsheets work fine with six jobs open. At sixty, they break. Supervisors walk the floor to find job status, planners act on data that's twelve hours old, labor times get written down late (or not at all), and clipboard QA can't really tell you whether the assembly matches the model. The information exists — you just can't act on it fast enough.
**Three pains (cards):**
- **Stale visibility.** You find out where a job stands at the end of the shift, not while you can still do something about it.
- **Labor you can't trust.** Hand-written times are slow and skipped, so costing and quoting are guesswork.
- **QA that misses the part.** A 2D checklist can't verify a built weldment against intent — rework shows up late.
...
- AR-assisted inspection right where the work happens.
- Sign-offs and defect-pattern views build a real quality record.
**Honesty line (small):** Deeper scan-to-model comparison is in active development.
**CTA:** See the 3D/AR QA flow in a demo

### How it works
**Headline:** Live in weeks, not quarters.
1. **Model your routings.** Define each product's stages and target times once. Versioned, drag-to-reorder.
2. **Operators run & inspect.** Clock into each stage on the phone or web; view the 3D model and flag defects in AR.
3. **See it all, live.** Real-time dashboard, Live Stage Status, QA results and labor reports — no refresh.

### Live floor visibility
**Headline:** Your whole floor, on one live screen.
**Body:** The dashboard shows work orders, active operators, output and labor efficiency at a glance — and a Live Stage Status board that lists who's working, on which order and stage, how long they've been on it, and at which station. It updates without a refresh, on the wall and in your pocket.
**CTA:** Book a demo

### Operator app
**Headline:** A real tool in the operator's hand.
**Body:** If reporting feels like paperwork, it won't happen. The operator app makes it one tap: see your assigned work, clock in and out per stage, pull up the 3D/AR reference, and keep working in the dead zones of the floor with an offline queue that syncs when you're back in range.
...
### Pricing teaser
**Headline:** Simple pricing that doesn't punish you for adding operators.
**Body:** Per-site plans banded by operator count — so every operator can clock in without running up the bill. Start with a low-risk pilot on one line.
**CTA:** See pricing
...
Operators clock into a specific stage of a specific work order in one tap. Idle, break and rework are captured, not guessed. Supervisors can review and correct entries, and compare actual vs. target time by stage — the accurate labor data that finally makes job costing and quoting possible.

**3D/AR quality & assembly inspection**
Inspect a built assembly against the actual 3D model and pin defects to the geometry — type, severity, measurements, tolerances — with AR-assisted inspection at the station. Sign-offs and defect-pattern views build a real quality record. *(Deeper scan-to-model comparison is in active development.)*

**Work orders & kanban**
A real lifecycle: draft → pending → in-progress → completed, with priorities, due dates, line assignment, order dependencies and batch operations — plus a kanban board to move work the way the floor moves.

**Process & routing templates**
Model each product's routing once: ordered stages with target times, versioned and drag-to-reorder. The same part gets built the same way every time, and actual-vs-target is comparable by stage.

**Operator mobile app**
Assigned work, one-tap clock in/out, 3D/AR reference, and an offline queue for the dead zones of the floor. iOS and Android.

**Roles, access & audit**
Role-based access for managers, supervisors and operators, with an audit log of who changed what.

**Built to fit your stack**
An Angular web portal, a React Native operator app, and a documented REST + real-time (WebSocket) API — deployed on standard cloud infrastructure.
...
- Sign off the inspection; recurring issues surface in defect-pattern views.
**Section — Why it matters:** Fewer escapes, less rework, and a quality record tied to the actual part — not a binder of checklists.
**Honesty line:** Today: model-based inspection, geometry-pinned defects, AR viewing, sign-offs and defect patterns. In active development: automated scan-to-model comparison.
**CTA:** See the 3D/AR QA flow live

---

## 4. Pricing page

**Headline:** Pricing that scales with your shop, not against it.
**Subhead:** Per-site plans banded by operator count. Start with a pilot, go live in weeks.
**Note for the team:** *the figures below are placeholders to confirm before publishing — or replace the numbers with a single "Talk to us" CTA until pricing is locked.*

| Plan | Best for | Includes |
|---|---|---|
| **Pilot** | Proving value on one line | One line, up to ~10 operators, core tracking & dashboard. 30–60 days. |
| **Starter** | A single small shop | One site, up to ~15 operators, web + mobile, kanban, reports. |
| **Growth** | A busy multi-line shop | One site, up to ~50 operators, 3D/AR QA, inspection, audit log. |
| **Plant** | Larger or multi-site | Multi-site, 50+ operators, priority support, SSO. |

**Line under table:** Every plan includes the web portal, the operator app, and the documented API. No per-seat charge that discourages operators from clocking in.
**CTA:** Get a quote / Book a demo

**Pricing FAQ:**
- **How is it priced?** By site, banded by how many operators you have — not per named user.
- **Is there a setup fee?** A one-time onboarding helps us model your routings and get operators live fast.
- **Can we start small?** Yes — a pilot on a single line proves the value before you roll out.
- **Annual or monthly?** Both; annual plans are discounted.
...
- **What's the 3D/AR part?** Operators inspect built assemblies against the actual 3D model and pin defects to the geometry, with AR viewing at the station.
- **Web or mobile?** Both — a web portal for managers and supervisors, an operator app (iOS/Android) for the floor, one live data source.
- **Is there an API?** Yes, a documented REST + real-time (WebSocket) API.
...
- **Primary buttons:** Book a demo · Get a quote · Watch the 90-sec demo
- **Demo form fields:** Name · Work email · Company · Shop size (operators) · What are you using today? *(Paper/Spreadsheets / An ERP / Another tool)*
- **Form button:** Book my demo
- **Form success:** Thanks — we'll be in touch within one business day to set up your demo.
- **Newsletter (optional):** Get one practical floor-visibility tip a month. *Button:* Subscribe
- **404:** This page took an unplanned offline break. *Button:* Back to home
- **Cookie/notice (if needed):** We use minimal analytics to improve the site. *Button:* Got it
